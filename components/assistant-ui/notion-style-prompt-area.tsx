...
interface NotionStylePromptAreaProps {
  ...
  onScrollToMessage?: (messageId: string, threadId: string) => void;
}
...
function promptToBlock(prompt: PromptItem, existingBlock?: NotionBlock): NotionBlock {
  ...
  const block = {
    ...
    sourceThreadId: prompt.sourceThreadId,
    sourceMessageRole: prompt.sourceMessageRole,
    sourceMessageTimestamp: prompt.sourceMessageTimestamp,
  };
  
  // 如果有来源消息信息，记录日志
  if (block.sourceMessageId) {
    console.log("[Link] [NotionPromptArea] promptToBlock: Preserving source message info:", {
      promptId: prompt.id,
      blockId: block.id,
      sourceMessageId: block.sourceMessageId,
      sourceThreadId: block.sourceThreadId,
      sourceMessageRole: block.sourceMessageRole,
      sourceMessageTimestamp: block.sourceMessageTimestamp,
    });
  }
...
function blockToPrompt(block: NotionBlock, originalPrompt: PromptItem): PromptItem {
  ...
  if (!block.content || !block.content.trim()) {
    ...
    if (prompt.sourceMessageId) {
      console.log("[Link] [NotionPromptArea] blockToPrompt (empty): Preserving source message info:", {
        blockId: block.id,
        promptId: prompt.id,
        sourceMessageId: prompt.sourceMessageId,
        sourceThreadId: prompt.sourceThreadId,
  ...
  // 如果有来源消息信息，记录日志
  if (prompt.sourceMessageId) {
    console.log("[Link] [NotionPromptArea] blockToPrompt: Preserving source message info:", {
      blockId: block.id,
      promptId: prompt.id,
      sourceMessageId: prompt.sourceMessageId,
      sourceThreadId: prompt.sourceThreadId,
...
export function NotionStylePromptArea({
  ...
  useEffect(() => {
    const handleGlobalKeyDown = (e: KeyboardEvent) => {
      ...
      allBlockElements.forEach((blockEl) => {
        ...
        try {
          // 更准确地检测交集：检查选择的起始或结束节点是否在这个 block 内
          const startInBlock = contentEl.contains(selectionRange.startContainer) || 
          ...
          const endInBlock = contentEl.contains(selectionRange.endContainer) || 
          ...
          const blockInSelection = selectionRange.intersectsNode(contentEl) ||
                                   (selectionRange.startContainer.compareDocumentPosition(contentEl) & Node.DOCUMENT_POSITION_CONTAINS) ||
                                   (selectionRange.endContainer.compareDocumentPosition(contentEl) & Node.DOCUMENT_POSITION_CONTAINS);
          
          if (startInBlock || endInBlock || blockInSelection) {
            affectedBlocks.push({ id: blockId, element: blockEl, contentEl });
          }
        } catch (err) {
          console.warn("[NotionPromptArea] Error checking block intersection:", err);
          // 如果检测失败，尝试更宽松的检测
          try {
      ...
      if (affectedBlocks.length > 0) {
        ...
        // 对每个受影响的 block 处理删除
        console.log("[NotionPromptArea] Current state:", {
        ...
        affectedBlocks.forEach(({ id, contentEl }) => {
          console.log("[NotionPromptArea] Processing affected block:", {
            blockId: id,
          ...
          if (!block) {
            console.warn("[NotionPromptArea] Block not found in blocks array:", {
              blockId: id,
              availableBlockIds: blocks.map(b => b.id),
            });
            return;
          }
          if (!contentEl) {
            console.warn("[NotionPromptArea] Content element not found for block:", id);
            return;
          }
          
          // 找到对应的 prompt（如果不存在，说明这是一个新创建的空 block，还没有对应的 prompt）
          const promptIndex = prompts.findIndex(p => p.id === id);
          let prompt: PromptItem | null = null;
          
          if (promptIndex === -1) {
            console.warn("[NotionPromptArea] Prompt not found for block, this might be a new empty block:", {
              blockId: id,
              availablePromptIds: prompts.map(p => p.id),
              blockExists: !!block,
              blockContent: block.content.substring(0, 50),
            });
            
            // 如果 block 有内容，创建一个新的 prompt
            if (block.content && block.content.trim()) {
              console.log("[NotionPromptArea] Creating new prompt for block with content:", id);
              prompt = {
                id: block.id,
                title: block.type === "toggle" 
                  ? block.content.split("\n")[0] || ""
              ...
              // 如果 block 是空的，跳过（不应该删除空 block，除非用户明确删除）
              console.log("[NotionPromptArea] Block is empty, skipping:", id);
              return;
            }
          } else {
          ...
          console.log("[NotionPromptArea] Found block and prompt:", {
            blockId: id,
            blockContentLength: block.content.length,
            blockContentPreview: block.content.substring(0, 30),
            promptIndex,
          });
          ...
          try {
            ...
            console.log("[NotionPromptArea] Calculating offsets for block:", {
              blockId: id,
              blockContentLength: block.content.length,
              startContainerNodeName: selectionRange.startContainer.nodeName,
              endContainerNodeName: selectionRange.endContainer.nodeName,
              startContainerInBlock: contentEl.contains(selectionRange.startContainer),
              endContainerInBlock: contentEl.contains(selectionRange.endContainer),
            });
            ...
            if (contentEl.contains(selectionRange.startContainer) || 
              ...
              try {
                ...
                console.log("[NotionPromptArea] Calculated startOffset:", { blockId: id, startOffset });
              } catch (err) {
                console.warn("[NotionPromptArea] Error calculating startOffset:", err, { blockId: id });
                // 如果设置失败，使用 0
                startOffset = 0;
              }
            } else {
              // 如果选择的开始不在这个 block 内，说明选择从这个 block 之前开始
              startOffset = 0;
              console.log("[NotionPromptArea] Selection starts before this block, using startOffset = 0:", id);
            }
            ...
            if (contentEl.contains(selectionRange.endContainer) || 
              ...
              try {
                ...
                console.log("[NotionPromptArea] Calculated endOffset:", { blockId: id, endOffset });
              } catch (err) {
                console.warn("[NotionPromptArea] Error calculating endOffset:", err, { blockId: id });
                // 如果设置失败，使用 block 内容的长度
                endOffset = block.content.length;
              }
            } else {
              // 如果选择的结束不在这个 block 内，说明选择延续到这个 block 之后
              endOffset = block.content.length;
              console.log("[NotionPromptArea] Selection ends after this block, using endOffset = block.content.length:", {
                blockId: id,
                endOffset: block.content.length,
              });
            }
            ...
            console.log("[NotionPromptArea] Final offsets for block:", {
              blockId: id,
              startOffset,
              endOffset,
              blockContentLength: block.content.length,
              willDeleteEntireBlock: startOffset === 0 && endOffset >= block.content.length,
              willDeletePartial: startOffset < endOffset && !(startOffset === 0 && endOffset >= block.content.length),
            });
            ...
            if (startOffset === 0 && endOffset >= block.content.length) {
              ...
              // 如果 prompt 不存在（新创建的），使用 block.id 作为 index 的占位符
              const actualIndex = promptIndex !== -1 ? promptIndex : prompts.length;
              blocksToDelete.push({ id, prompt, index: actualIndex });
              ...
              if (!newContent.trim()) {
                ...
                // 标记为更新
                // 如果 prompt 不存在（新创建的），oldPrompt 应该是新创建的 prompt
                const oldPromptForUpdate = prompt || {
                  id: block.id,
                  title: "",
                  content: [],
                  isIncluded: block.isIncluded ?? true,
                ...
                console.log("[NotionPromptArea] Marking block for update:", {
                  blockId: id,
                  isNewPrompt: promptIndex === -1,
                  oldContentLength: oldPromptForUpdate.content.length,
                  newContentLength: newPrompt.content.length,
                });
                // 如果 prompt 不存在（新创建的），使用 block.id 作为 index 的占位符
                const actualIndex = promptIndex !== -1 ? promptIndex : prompts.length;
                blocksToUpdate.push({ id, oldPrompt: oldPromptForUpdate, newPrompt, index: actualIndex });
              }
            } else {
              console.warn("[NotionPromptArea] Invalid offset range, skipping block:", {
                blockId: id,
                startOffset,
                endOffset,
              });
        ...
        // 批量处理删除和更新
        console.log("[NotionPromptArea] Final blocks to process:", {
          deleteCount: blocksToDelete.length,
          updateCount: blocksToUpdate.length,
          blocksToDeleteIds: blocksToDelete.map(b => b.id),
          blocksToUpdateIds: blocksToUpdate.map(b => b.id),
        });
        ...
        if (blocksToDelete.length > 0 || blocksToUpdate.length > 0) {
          ...
          console.log("[NotionPromptArea] Batch actions created:", {
            totalActions: batchActions.length,
            deleteActions: batchActions.filter(a => a.type === "delete").length,
            updateActions: batchActions.filter(a => a.type === "update").length,
          });
          
          // 如果有多个操作，使用 batch 类型；否则使用单个操作
          if (batchActions.length > 1) {
            console.log("[NotionPromptArea] Dispatching batch operation event:", {
              deleteCount: blocksToDelete.length,
              updateCount: blocksToUpdate.length,
              totalActions: batchActions.length
            });
            // 同时从 blocks 中删除对应的 blocks，避免 useEffect 重新创建 prompts
            const blocksToDeleteIds = blocksToDelete.map(b => b.id);
            setBlocks(prev => prev.filter(b => !blocksToDeleteIds.includes(b.id)));
            ...
            if (typeof window !== "undefined") {
              ...
              console.log("[NotionPromptArea] Batch operation event dispatched");
            } else {
              console.warn("[NotionPromptArea] window is undefined, cannot dispatch event");
            }
          } else if (batchActions.length === 1) {
            // 单个操作，直接处理
            const action = batchActions[0];
            console.log("[NotionPromptArea] Single operation:", action.type);
            if (action.type === "delete") {
              console.log("[NotionPromptArea] Calling onDeletePrompt for:", action.prompt.id);
              // 同时从 blocks 中删除对应的 block，避免 useEffect 重新创建 prompt
              setBlocks(prev => prev.filter(b => b.id !== action.prompt.id));
              onDeletePrompt(action.prompt.id);
            } else if (action.type === "update") {
              console.log("[NotionPromptArea] Calling onUpdatePrompt for:", action.oldPrompt.id);
              // updatePrompt 现在支持创建新 prompt（如果不存在）
            ...
            console.warn("[NotionPromptArea] No actions to process, this should not happen");
          }
        } else {
          console.warn("[NotionPromptArea] No blocks to delete or update, but affectedBlocks.length > 0. This might indicate a bug in offset calculation.");
        }
  ...
  }, [blocks, prompts, onDeletePrompt, onUpdatePrompt]);
  ...
  const lastSyncedPromptsRef = useRef<string>("");
  ...
  useEffect(() => {
    ...
    // 计算 blocks 和 prompts 的签名，用于检测真正的变化
    const blocksSignature = JSON.stringify(blocks.map(b => ({ id: b.id, content: b.content, isIncluded: b.isIncluded })));
    const promptsSignature = JSON.stringify(prompts.map(p => ({ id: p.id, title: p.title, content: p.content })));
    
    // 如果签名没有变化，跳过同步
    if (blocksSignature === lastSyncedBlocksRef.current && promptsSignature === lastSyncedPromptsRef.current) {
      return;
    }
    ...
    const timeoutId = setTimeout(() => {
      ...
      try {
        ...
        prompts.forEach((prompt) => {
          ...
          if (!block) {
            // Block 被删除，但 prompt 还存在，标记为删除
            promptsToDelete.push(prompt.id);
          }
        });
        
        // 批量删除 prompts（避免多次触发更新）
        if (promptsToDelete.length > 0) {
          console.log("[NotionPromptArea] Deleting prompts for removed blocks:", promptsToDelete);
          promptsToDelete.forEach(id => onDeletePrompt(id));
        }
        
        // 同步 blocks 到 prompts（只同步内容变化，不处理 isExpanded）
        const updates: Array<{ id: string; title: string; content: string[] }> = [];
        ...
        // 批量更新 prompts（避免多次触发更新）
        if (updates.length > 0) {
          console.log("[NotionPromptArea] Updating prompts:", updates.length);
          updates.forEach(update => {
            onUpdatePrompt(update.id, {
              title: update.title,
        ...
        // 更新签名
        lastSyncedBlocksRef.current = blocksSignature;
        lastSyncedPromptsRef.current = promptsSignature;
      } finally {
        // 清除同步标志
        setTimeout(() => {
