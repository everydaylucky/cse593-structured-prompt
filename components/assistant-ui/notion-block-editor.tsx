...
import { ChevronRight, Plus, GripVertical, Check, X, Link } from "lucide-react";
...
export function NotionBlockEditor({
  ...
  const handleScrollToMessage = useCallback(() => {
    console.log("[Link] [NotionBlockEditor] handleScrollToMessage called:", {
      blockId: id,
    ...
    if (block.sourceMessageId && onScrollToMessage) {
      // 如果 sourceThreadId 不存在，尝试使用当前线程 ID
      const currentThreadId = getCurrentThreadId();
      const threadId = block.sourceThreadId || currentThreadId || "";
      
      console.log("[Link] [NotionBlockEditor] Thread ID resolution:", {
        sourceThreadId: block.sourceThreadId,
        currentThreadId,
      ...
      if (threadId) {
        console.log("[Link] [NotionBlockEditor] Calling onScrollToMessage:", {
          messageId: block.sourceMessageId,
          threadId,
        });
        onScrollToMessage(block.sourceMessageId, threadId);
      } else {
        // 如果连当前线程 ID 都没有，仍然尝试跳转（可能会失败，但至少尝试）
        console.warn("[Link] [NotionBlockEditor] No thread ID available for message:", {
          messageId: block.sourceMessageId,
          blockId: id,
          sourceThreadId: block.sourceThreadId,
      ...
      console.warn("[Link] [NotionBlockEditor] Cannot scroll to message:", {
        hasSourceMessageId: !!block.sourceMessageId,
        hasOnScrollToMessage: !!onScrollToMessage,
        blockId: id,
      });
    }
  }, [block.sourceMessageId, block.sourceThreadId, onScrollToMessage, id]);
  
  // 检查是否有来源消息信息（只要有 sourceMessageId 就显示图标）
  const hasSourceMessage = !!block.sourceMessageId;
  
  // 添加日志用于调试图标显示
  useEffect(() => {
    if (hasSourceMessage) {
      console.log("[Link] [NotionBlockEditor] Block has source message, link icon should be visible:", {
        blockId: id,
        sourceMessageId: block.sourceMessageId,
        sourceThreadId: block.sourceThreadId,
  ...
  }, [hasSourceMessage, id, block.sourceMessageId, block.sourceThreadId, block.sourceMessageRole, block.sourceMessageTimestamp]);
  ...
  const handleContentKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    if ((e.key === "Delete" || e.key === "Backspace")) {
      ...
      if (selectedText && selection && selection.rangeCount > 0) {
        ...
        // 检查选择是否跨越了多个 block（通过检查是否有其他 block 元素在选择范围内）
        ...
        for (let i = 0; i < allBlockElements.length; i++) {
          const blockEl = allBlockElements[i];
          if (blockEl.getAttribute('data-block-id') === id) continue; // 跳过当前 block
          ...
          if (contentEl) {
            try {
              if (range.intersectsNode(contentEl) || 
                  ...
                  range.commonAncestorContainer.contains(contentEl)) {
        ...
        if (spansMultipleBlocks) {
          // 选择跨越了多个 block，让全局处理器来处理
          console.log("[NotionBlockEditor] Selection spans multiple blocks, letting global handler process:", {
            blockId: id,
        ...
        if (isWithinBlock) {
          // 选中的文本只在当前 block 内
          console.log("[NotionBlockEditor] Selected text within block:", {
            blockId: id,
          ...
          e.preventDefault();
          e.stopPropagation(); // 阻止事件冒泡，避免全局处理器也处理
          ...
          if (contentElement.contains(range.startContainer) || range.startContainer === contentElement) {
            ...
            try {
              ...
              console.warn("[NotionBlockEditor] Error calculating start offset:", err);
            }
          }
          ...
          if (contentElement.contains(range.endContainer) || range.endContainer === contentElement) {
            ...
            try {
              ...
              console.warn("[NotionBlockEditor] Error calculating end offset:", err);
            }
          }
          ...
          // 选择不在当前 block 内，让全局处理器处理
          console.log("[NotionBlockEditor] Selection not in this block, letting global handler process:", {
            blockId: id,
          });
          // 不阻止默认行为，让事件冒泡到全局处理器
        }
