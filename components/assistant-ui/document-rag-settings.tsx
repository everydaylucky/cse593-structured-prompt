...
import { Settings, FileText, Search, Sparkles } from "lucide-react";
...
import {
  ...
  type DocumentRAGModeConfig,
} from "@/lib/document-rag-mode-storage";

export function DocumentRAGSettings() {
  const [open, setOpen] = useState(false);
  const [config, setConfig] = useState<DocumentRAGModeConfig>({
    mode: 'smart',
    smartThreshold: {
      maxTextLength: 50000,
    },
  ...
  // Calculate page estimate (assuming ~2500-3000 characters per page)
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Document Reference Mode</DialogTitle>
          <DialogDescription>
            Choose how documents are processed when referenced in chat
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            ...
            <Select value={config.mode} onValueChange={handleModeChange}>
              <SelectTrigger className="text-left [&>span]:text-left [&>span]:justify-start">
          ...
          {config.mode === 'smart' && (
            <div className="space-y-3 p-3 border rounded-lg bg-muted/50">
              <div className="space-y-2">
                <Label htmlFor="maxTextLength" className="text-sm font-semibold">
